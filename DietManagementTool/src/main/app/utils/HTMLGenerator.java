...
public class HTMLGenerator {
    ...
    public static void generateHTML(List<DateFood> foods, String username) {
        ...
        generateLinearRegressionChart(htmlContent, foods);

        generateLineChartWithGoogleCharts(htmlContent, foods);
    ...
    private static void initiateHTMLDocument(StringBuilder htmlContent, String username) {
        htmlContent.append("<!DOCTYPE html>\n<html>\n<head>\n").append("<title>" + username + "</title>\n").append(generateStyles()).append("</head>\n<body>\n");
    }
    ...
    private static void writeToFile(String htmlContent, String username) {
        try {
            ...
            File htmlFile = new File(reportsDir, username + ".html");
            FileWriter fileWriter = new FileWriter(htmlFile);
            fileWriter.write(htmlContent);
            fileWriter.close(); // Ensure the FileWriter is closed properly
    ...
    public static void generateMealTypeTable(StringBuilder htmlContent, List<DateFood> foods, String mealType) {
        List<DateFood> filteredFoods = foods.stream().filter(food -> food.getMealType().equalsIgnoreCase(mealType)).collect(Collectors.toList());
    ...
    private static void generateLinearRegressionChart(StringBuilder htmlContent, List<DateFood> dateFoods) {
        Map<LocalDate, Double> dailyCaloriesMap = FoodUtils.calculateDailyCalories(dateFoods);
        double[] y = dailyCaloriesMap.values().stream().mapToDouble(Double::doubleValue).toArray();
        LinearRegression lr = new LinearRegression(y);

        htmlContent.append("<div id=\"regressionChart\" style=\"width: 100%; max-width: 800px; height: auto; aspect-ratio: 16 / 9; margin: 10px;\"></div>\n").append("<script type=\"text/javascript\" src=\"https://www.gstatic.com/charts/loader.js\"></script>\n").append("<script type=\"text/javascript\">\n").append("google.charts.load('current', {'packages':['corechart']});\n").append("google.charts.setOnLoadCallback(drawChart);\n").append("function drawChart() {\n").append("var data = google.visualization.arrayToDataTable([\n").append("['Day', 'Calories', {type: 'string', role: 'annotation'}],\n");

        int dayIndex = 1;
        for (Double calorie : y) {
            htmlContent.append("[").append(dayIndex).append(", ").append(calorie).append(", null],\n");
            dayIndex++;
        }

        // Predict future values
        int totalDays = lr.getAmountOfDaysInData() + 5; // Predict next 5 days
        for (int i = lr.getAmountOfDaysInData() + 1; i <= totalDays; i++) {
            double predictedValue = lr.extrapolate(i);
            htmlContent.append("[").append(i).append(", ").append(predictedValue).append(", 'Predicted'],\n");
        }

        htmlContent.append("]);\n").append("var options = {\n").append("title: 'Calorie Intake Trend and Predictions',\n").append("series: {1: {curveType: 'function'}},\n").append("legend: { position: 'bottom' }\n").append("};\n").append("var chart = new google.visualization.LineChart(document.getElementById('regressionChart'));\n").append("chart.draw(data, options);\n").append("}\n").append("</script>\n");
    }
